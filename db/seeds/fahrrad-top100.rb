...
class Analyser
  ...
  def run
    ...
    selector = find_selector_by_feed_url
    if !selector
      puts "  FEHLER KEIN selektor fuer #{@feed_url} gefunden"
    end

    source = FeedSource.new(url: @feed_url, name: title, full_text_selector: selector)
    source.value = 1
    if source.save
...
text = File.read 'db/seeds/import.txt'
missing = text.split("\n").map(&:strip).map(&:downcase)
missing.each do |domain|
  url = "http://#{domain}"
  Analyser.new(url).run()
end
